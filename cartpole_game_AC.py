...
class Actor(nn.Module):
    ...
    def learn(self, s, a, value):
        ...
        loss = -log_prob * value  # change td_error to without_td_error************---->value
        loss.backward()
        self.optimizer.step()
...
class Critic(nn.Module):
    ...
    def learn(self, s, r, s_):
        ...
        loss.backward()
        self.optimizer.step()
        return v.item()  # return the value of the state
...
def create_animation(frames):
    ...
    animation.FuncAnimation(fig, animate, frames=len(frames), interval=50)
    plt.show()
...
for i_episode in range(MAX_EPISODE):
    ...
    while True:
        ...
        if done: r = -20
        track_r.append(r)
        value = critic.learn(s, r, s_)
        actor.learn(s, a, value)
        s = s_
        t += 1
